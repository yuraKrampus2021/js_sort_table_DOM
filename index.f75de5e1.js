...
function sortList(colect, indx) {
    const out = Array.from(colect).sort((a, b)=>{
        if (!isNaN(cleanCurrency(a.children[indx].textContent))) return cleanCurrency(a.children[indx].textContent) - cleanCurrency(b.children[indx].textContent);
        return a.children[indx].textContent.localeCompare(b.children[indx].textContent);
    });
    return out;
}
